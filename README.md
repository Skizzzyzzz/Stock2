...
| `bind() failed on port 8081` | Something else is using port 8081. Find and close it, or change the port number in `src/main.cpp` and rebuild |/n
| `[Yahoo] Warning: could not obtain crumb` | No internet connection, or Yahoo Finance is temporarily down. The frontend will fall back to cached stock data from the last successful fetch | /n
